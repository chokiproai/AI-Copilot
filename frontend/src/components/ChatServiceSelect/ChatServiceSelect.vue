...
const handleChangeSydneyConfig = (config: SydneyConfig) => {
  ...
  if (!config.baseUrl.startsWith('https://')) {
    message.error('Vui lòng điền vào liên kết chính xác bắt đầu bằng https');
    return;
  }
...
<template>
  <NModal class="w-11/12 lg:w-[900px]" v-model:show="isShowChatServiceSelectModal" preset="card" title="Cài đặt máy chủ trò chuyện">
    <NTable striped>
      <tbody>
        <tr v-for="(config, index) in sydneyConfigs" :key="index">
          <td>
            ...
            <NInput class="lg:hidden" v-if="config.isCus" v-model:value="config.baseUrl" placeholder="Liên kết máy chủ trò chuyện tùy chỉnh" @change="handleChangeSydneyConfig(config)"></NInput>
          </td>
          <td class="hidden lg:table-cell">
            <NInput v-if="config.isCus" v-model:value="config.baseUrl" placeholder="Liên kết máy chủ trò chuyện tùy chỉnh" @change="handleChangeSydneyConfig(config)"></NInput>
          ...
          <td>
            ...
            <div v-else-if="config.baseUrl" class="flex justify-center items-center flex-wrap gap-2" @click="checkSydneyConfig(config)">
              <NTag v-if="config.isUsable === false" type="error" class="cursor-pointer">không có sẵn</NTag>
              <NTag v-if="config.delay" type="success" class="cursor-pointer">{{ config.delay }} ms</NTag>
            </div>
          </td>
          <td>
            <div class="flex justify-center items-center flex-wrap gap-2">
              <NButton class="hidden lg:table-cell" secondary @click="checkSydneyConfig(config)">Phát hiện</NButton>
              <NButton v-if="config.baseUrl === selectedSydneyBaseUrl" secondary type="success">hiện hành</NButton>
              <NButton v-else secondary type="info" @click="selectSydneyConfig(config)">chọn</NButton>
            </div>
          </td>
        </tr>
      </tbody>
